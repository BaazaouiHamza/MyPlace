...
const getPlacesByUserId = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("Could not find places for the provided user.");
    return next(error);
  }
  if (!places) {
    return next(
...
const createPlace = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return next(
      HttpError("Invalid inputs passed,please cheack your data.", 422)
    );
  }
...
const updatePlace = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    const error = new HttpError(
      "Invalid inputs passed,please cheack your data.",
      422
    );
    next(error);
  }
